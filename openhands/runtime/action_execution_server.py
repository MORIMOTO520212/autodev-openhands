...
class ActionExecutor:
    ...
    async def _init_bash_commands(self):
        INIT_COMMANDS = [
            'git config --file ./.git_config user.name "y.morimoto" && git config --file ./.git_config user.email "y.morimoto@sonicmoov.com" && alias git="git --no-pager" && export GIT_CONFIG=$(pwd)/.git_config'
            if os.environ.get('LOCAL_RUNTIME_MODE') == '1'
            else 'git config --global user.name "y.morimoto" && git config --global user.email "y.morimoto@sonicmoov.com" && alias git="git --no-pager"'
        ]
